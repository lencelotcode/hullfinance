...
export default function Dashboard() {
  ...
  const recentExp = [...filteredExpenses]
    .sort((a, b) => b.date.localeCompare(a.date))
    .slice(0, 5);
  const recentInc = [...filteredIncomes]
    .sort((a, b) => b.date.localeCompare(a.date))
    .slice(0, 5);
  ...
  return (
    <div>
      ...
      <div className="hf-stat-grid">
        ...
        <StatCard
          label="Expenses"
          value={`-${fmt(totalExpenses, state.currency)}`}
          colorClass="red"
          icon={ArrowDownRight}
          iconColor="#B03030"
        />
        <StatCard
          label="Net"
          value={`${netBalance >= 0 ? "+" : "-"}${fmt(Math.abs(netBalance), state.currency)}`}
          colorClass={netBalance >= 0 ? "green" : "red"}
          icon={Wallet}
          iconColor={netBalance >= 0 ? "#5A9E6F" : "#B03030"}
        />
        <StatCard
          label="Accounts"
          value={fmt(totalAccountBalance, state.currency)}
          colorClass="blue"
          icon={PiggyBank}
          iconColor="#5A8FAE"
        />
      ...
      <div className="hf-stat-grid cols-3 hf-stat-grid-last">
        <StatCard
          ...
          value={fmt(totalLoanOwed, state.currency)}
          colorClass="amber"
          icon={CreditCard}
          iconColor="#C9973A"
        />
        <StatCard
          ...
          value={fmt(totalDebtOwed, state.currency)}
          colorClass="amber"
          icon={Receipt}
          iconColor="#C9973A"
        />
        <StatCard
          ...
          value={fmt(upcomingBillsTotal, state.currency)}
          colorClass="red"
          icon={AlertTriangle}
          iconColor="#B03030"
        />
      ...
      <div className="hf-two-col" style={{ marginTop: "20px" }}>
              ...
              recentExp.map((e) => (
                <div key={e.id} className="hf-tx-item">
                  ...
                  <div className="hf-tx-info">
                    ...
                    <div className="hf-tx-meta">
                      {e.date}
                      {e.source ? " · " + e.source : ""}
                      {e.note ? " · " + e.note : ""}
                    </div>
                  </div>
                  <span className="hf-tx-amount expense">
        ...
        <div className="hf-panel">
          <div className="hf-section-header">
            ...
            <span className="hf-section-count">
              {filteredIncomes.length} this month
            </span>
          </div>
          <div className="hf-tx-list">
            {recentInc.length === 0 ? (
              ...
              recentInc.map((i) => (
                <div key={i.id} className="hf-tx-item">
                  ...
                  <span className="hf-tx-amount income">
                    +{fmt(i.amount, state.currency)}
                  </span>
                </div>
              ))
            )}
      ...
      {chartData.length > 0 && (
        <div className="hf-panel" style={{ marginTop: "20px" }}>
          <div className="hf-section-title" style={{ marginBottom: "16px" }}>
